...
class FrontUiApplicationTests {
    private static final Logger TEST_LOGGER = (Logger) LoggerFactory.getLogger(FrontUiApplicationTests.class);
    ...
    @Test
    @DisplayName("Actuator prometheus доступен без логина")
    ...
    void traceIdsAreAddedToMdc() {
        ...
        var span = tracer.nextSpan().name("front-ui-mdc-test").start();
        try (Tracer.SpanInScope ignored = tracer.withSpan(span)) {
            TEST_LOGGER.info("front-ui mdc test");
        } finally {
            span.end();
